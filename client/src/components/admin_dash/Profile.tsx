...
const CompnayForm: React.FC<CompanyFormProps> = ({ open, setOpen }) => {
  ...
  const { adminInfo, searchTerm } = state
  ...
  const onSubmit = async (data: AdminInfo) => {
    console.log(data);
    ...
    try {
      const res = await apiClient.put(`api/admin/update-profile`, { data })
      if (res.status == 400) {
        console.log(res);
      }
      else {
...
const CompanyProfile: React.FC = () => {
  ...
  // const Jobs = jobs?.filter(job => job.recruiterId === adminInfo._id);
